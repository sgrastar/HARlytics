...
export const detectCDN = (entry) => {
  ...
  const serverHeader =
    headers.find((header) => header.name.toLowerCase() === "server")?.value ||
    "";

  // Check for CDN-specific headers
  const hasCloudflareCDN = headers.some(
    (header) =>
  ...
  const hasAkamaiCDN = headers.some(
    (header) =>
  ...
  const hasFastlyCDN = headers.some(
    (header) =>
  ...
  const hasGoogleCDN = headers.some(
    (header) =>
  ...
  const viaHeader =
    headers.find((header) => header.name.toLowerCase() === "via")?.value || "";
  ...
  if (domain.includes("akamaiedge") || domain.includes("akamai.net"))
    return "Akamai";
  ...
  switch (provider) {
    case "Cloudflare":
      ...
      if ("cf-cache-status" in headerMap) {
        ...
        const cfCacheHit = ["HIT", "STALE", "UPDATING", "REVALIDATED"].includes(
          cfStatus
        );
    ...
    case "Fastly":
      ...
      if ("x-served-by" in headerMap && "x-cache" in headerMap) {
        result.status = headerMap["x-cache"];
        const fastlyCacheHit = headerMap["x-cache"]
          .toLowerCase()
          .includes("hit");
    ...
    case "CloudFront":
      ...
      if ("x-cache" in headerMap) {
        result.status = headerMap["x-cache"];
        const cloudFrontCacheHit = headerMap["x-cache"]
          .toLowerCase()
          .includes("hit");
    ...
    case "Varnish":
      ...
      if ("x-cache" in headerMap) {
        result.status = headerMap["x-cache"];
        const varnishXCacheHit = headerMap["x-cache"]
          .toLowerCase()
          .includes("hit");
    ...
    case "Incapsula":
      ...
      if ("x-iinfo" in headerMap && "incap-cache-status" in headerMap) {
        ...
        const incapsulaHit =
          headerMap["incap-cache-status"].toLowerCase() === "hit";
    ...
    case "REDBOX":
      ...
      if ("red-cache" in headerMap) {
        ...
        result.isFromOrigin =
          headerMap["red-cache"].toLowerCase() === "miss" ||
    ...
    default:
      ...
      if ("x-cache" in headerMap) {
        result.status = headerMap["x-cache"];
        const genericCdnHit = headerMap["x-cache"]
          .toLowerCase()
          .includes("hit");
